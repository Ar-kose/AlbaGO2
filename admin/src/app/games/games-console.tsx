...
export function GamesConsole() {
  ...
  async function publishSelected() {
    ...
    setIsBusy(true);
    try {
      const published = await publishGameDefinition(selectedId, 'admin@local');
      await refresh(published.id);
      setMessage(`${published.title} yayina alindi.`);
    } catch (error) {
      const details = error instanceof Error ? error.message : 'unknown_error';
    ...
    <section className="console-layout">
      ...
      <article className="panel stack">
        ...
        <div className="button-row">
          ...
          <button className="secondary-button" disabled={isBusy || !selectedId} onClick={() => void publishSelected()} type="button">
            Publish
          </button>
